...
class PresenceSocket:
    ...
    def keepalive(self):
        ...
        self.send(dumps(
            {"msg":"register","data":{"ID":int(self._client.x_avkn_userid),"version":"v1.1","token":self._client.x_avkn_session,"friends":self._client.friends}}, separators=(',', ':'))
        )
        sleep(20)

        while True:
            # print(f"[{self._client.x_avkn_username}:{self._client.x_avkn_userid}] Sending Keep-Alive")
